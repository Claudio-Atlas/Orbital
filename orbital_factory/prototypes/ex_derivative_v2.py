"""
"Why eˣ Is Its Own Derivative" — Orbital Short v2
===================================================
MAJOR visual upgrade from v1:
  - Neon grid backgrounds on all graph scenes
  - Height/slope as animated vertical BARS (gauge style)
  - No plain white text — everything boxed/colored
  - Bigger graphs filling more of 9:16 frame
  - More animation, particles, glow effects
  - Zero dead space

Render:
  cd ~/Desktop/Orbital/orbital_factory && source venv/bin/activate
  PATH="/Library/TeX/texbin:$PATH" manim render prototypes/ex_derivative_v2.py ExDerivativeV2 \
    -o ex_derivative_v2.mp4 --format mp4 -r 1080,1920 --frame_rate 60 --flush_cache
"""
from manim import *
import numpy as np
import json
...
def _neon_grid(center, w, h, spacing=0.5, color=GRID_COL, opacity=0.25):
    """Subtle neon grid for graph backgrounds."""
    lines = VGroup()
    cx, cy = center[0], center[1]
    x = cx - w / 2
    while x <= cx + w / 2:
        l = Line([x, cy - h / 2, 0], [x, cy + h / 2, 0],
                 color=color, stroke_width=0.5, stroke_opacity=opacity)
        lines.add(l)
        x += spacing
    y = cy - h / 2
    while y <= cy + h / 2:
        l = Line([cx - w / 2, y, 0], [cx + w / 2, y, 0],
                 color=color, stroke_width=0.5, stroke_opacity=opacity)
        lines.add(l)
        y += spacing
    return lines
...
def _make_gauge(x_pos, max_height, label_text, color, center_y=0):
    """Create a vertical gauge bar with label."""
    # Background track
    track = RoundedRectangle(
        width=0.3, height=max_height, color=color,
        fill_color=BOX_FILL, fill_opacity=0.5,
        stroke_width=1.5, stroke_opacity=0.5,
        corner_radius=0.05,
    ).move_to([x_pos, center_y, 0])

    # Label
    lbl = Text(label_text, font_size=11, color=color, weight=BOLD)
    lbl.move_to([x_pos, center_y - max_height / 2 - 0.2, 0])

    return VGroup(track, lbl)


class ExDerivativeV2(Scene):
    def construct(self):
        ...
        # ═══════════════════════════════════════════════
        # SCENE 1: HOOK (4.7s) — Big equation, looping arrow
        # ═══════════════════════════════════════════════

        # Full-screen neon grid
        ...
        # Looping arrow (bigger, more visible)
        arrow = CurvedArrow(
        ...
        # Decorative exponential curves in background
        ...
        self.play(FadeIn(q_box), Write(q_text), run_time=0.3)

        # Pulse the arrow
        self.play(
            arrow.animate.set_color(GREEN).scale(1.1),
            run_time=0.3
        )
        self.play(arrow.animate.scale(1 / 1.1), run_time=0.3)

        self.wait(max(0, TTS["hook"] - 2.0))

        self.play(FadeOut(VGroup(hook_eq, hook_box, q_text, q_box, arrow,
                                  bg_curve1, bg_curve2)), run_time=0.3)

        # ═══════════════════════════════════════════════
        # SCENE 2: DERIVATIVE REMINDER (9.5s) — Big graph, filled
        # ═══════════════════════════════════════════════
        ...
        self.play(FadeIn(title_box), FadeIn(title), run_time=0.3)

        # Bigger graph with neon grid
        graph_grid = _neon_grid([0, 0.5], 3.8, 4.0, 0.5, GRID_COL, 0.2)
        self.play(FadeIn(graph_grid), run_time=0.2)
        ...
        # "SLOPE" label that follows the dot
        def _get_slope_tag():
            x = x_rem.get_value()
            y = 0.3 * x ** 2 + 0.5
            s = 0.6 * x
        ...
        self.play(x_rem.animate.set_value(3.2), run_time=3.0, rate_func=smooth)
        self.play(x_rem.animate.set_value(1.8), run_time=2.0, rate_func=smooth)

        # Boxed caption at bottom
        ...
        self.play(FadeIn(cap_box), FadeIn(cap), run_time=0.3)

        self.wait(max(0, TTS["derivative_reminder"] - 6.5))

        self.play(FadeOut(VGroup(title, title_box, graph_grid, ax_r, curve_r,
                                  tang_r, dot_r, slope_tag, cap, cap_box)), run_time=0.3)

        # ═══════════════════════════════════════════════
        # SCENE 3: THE CLAIM — eˣ slope = height (8.7s)
        # Big graph + gauge bars
        # ═══════════════════════════════════════════════
        ...
        self.play(FadeIn(claim_box), FadeIn(claim_title), run_time=0.3)

        # Grid + axes
        g3_grid = _neon_grid([-0.3, 0.3], 3.0, 4.0, 0.5, GRID_COL, 0.2)
        self.play(FadeIn(g3_grid), run_time=0.2)
        ...
        self.play(FadeIn(lbl_box), FadeIn(label_e), run_time=0.2)

        # Gauge bars on the right side
        ...
        self.play(FadeIn(VGroup(gauge_h_track, gauge_s_track, h_lbl, s_lbl)), run_time=0.3)
        ...
        self.play(FadeIn(dot_e), Create(tang_e), Create(h_line), run_time=0.4)

        # Fill gauge bars to match (height and slope both = 2.7)
        bar_h = y_pt / 10.0 * 3.2  # scale to gauge height
        ...
        gauge_h_fill.move_to([1.65, 0.3 - 3.2 / 2 + bar_h / 2, 0])
        ...
        gauge_s_fill.move_to([2.0, 0.3 - 3.2 / 2 + bar_h / 2, 0])

        # Value labels on gauges
        ...
        self.play(FadeIn(match_bx), Write(match_text), run_time=0.3)

        self.wait(max(0, TTS["the_claim"] - 3.5))

        # Keep axes, clear extras for sliding scene
        ...
        # ═══════════════════════════════════════════════
        # SCENE 4: SLIDING DOT — live gauges (7.9s + 2s silence)
        # ═══════════════════════════════════════════════
        ...
        self.play(FadeIn(slide_bx), FadeIn(slide_title), run_time=0.2)
        ...
        # ═══════════════════════════════════════════════
        # SCENE 5: CONTRAST 2ˣ (6.8s) — Different color, gauge mismatch
        # ═══════════════════════════════════════════════

        c2_grid = _neon_grid([0, 0.3], 3.8, 4.5, 0.5, GRID_COL, 0.2)
        self.play(FadeIn(c2_grid), run_time=0.15)
        ...
        self.play(FadeIn(c2_bx), FadeIn(c2_title), run_time=0.2)

        ax_2 = _make_axes([0, 3.5, 1], [0, 10, 2], 2.6, 3.5, [-0.3, 0.2, 0])
        curve_2 = ax_2.plot(lambda x: 2 ** x, x_range=[0, 3.3],
                            color=ORANGE, stroke_width=3)
        self.play(Create(ax_2, run_time=0.3), Create(curve_2, run_time=0.4))
        ...
        self.play(FadeIn(VGroup(g2_ht, g2_st, g2_hl, g2_sl)), run_time=0.2)

        # At x=2: height=4.0, slope=2.77
        x_2 = 2.0
        y_2 = 2 ** x_2  # 4.0
        ...
        self.play(FadeIn(dot_2), Create(tang_2), Create(hl_2), run_time=0.3)
        ...
        self.play(GrowFromEdge(g2_hf, DOWN), GrowFromEdge(g2_sf, DOWN),
                  FadeIn(g2_hv), FadeIn(g2_sv), run_time=0.4)

        # ≠ symbol
        neq_2 = MathTex(r"\neq", font_size=30, color="#FF4444")
        neq_2.move_to([1.825, 0.2 + 1.0, 0])
        neq_bx = _box(neq_2, "#FF4444", "#2a0a0a", 0.4, 0.06)
        self.play(FadeIn(neq_bx), FadeIn(neq_2, scale=1.5), run_time=0.3)

        # "slope < height" at bottom
        ...
        self.play(FadeIn(lt_bx), Write(lt_text), run_time=0.3)

        self.wait(max(0, TTS["contrast_2x"] - 3.0))
        ...
        self.play(FadeIn(c3_grid), run_time=0.15)
        ...
        self.play(FadeIn(c3_bx), FadeIn(c3_title), run_time=0.2)

        ax_3 = _make_axes([0, 2.5, 0.5], [0, 10, 2], 2.6, 3.5, [-0.3, 0.2, 0])
        curve_3 = ax_3.plot(lambda x: 3 ** x, x_range=[0, 2.1],
                            color=CYAN, stroke_width=3)
        self.play(Create(ax_3, run_time=0.3), Create(curve_3, run_time=0.4))
        ...
        self.play(FadeIn(VGroup(g3_ht, g3_st, g3_hl, g3_sl)), run_time=0.2)

        x_3 = 1.5
        y_3 = 3 ** x_3  # ≈5.2
        s_3 = y_3 * np.log(3)  # ≈5.7
        ...
        self.play(FadeIn(dot_3), Create(tang_3), Create(hl_3), run_time=0.3)
        ...
        self.play(GrowFromEdge(g3_hf, DOWN), GrowFromEdge(g3_sf, DOWN),
                  FadeIn(g3_hv), FadeIn(g3_sv), run_time=0.4)

        neq_3 = MathTex(r"\neq", font_size=30, color="#FF4444")
        neq_3.move_to([1.825, 0.2 + 1.0, 0])
        neq_bx3 = _box(neq_3, "#FF4444", "#2a0a0a", 0.4, 0.06)
        self.play(FadeIn(neq_bx3), FadeIn(neq_3, scale=1.5), run_time=0.3)
        ...
        self.play(FadeIn(gt_bx), Write(gt_text), run_time=0.3)

        self.wait(max(0, TTS["contrast_3x"] - 2.8))
        ...
        # ═══════════════════════════════════════════════
        # SCENE 7: THE ANSWER — e ≈ 2.718 (9.8s + 2s silence)
        # ═══════════════════════════════════════════════

        ans_grid = _neon_grid([0, 0], FW, FH, 0.6, GRID_COL, 0.12)
        self.play(FadeIn(ans_grid), run_time=0.15)
        ...
        self.play(FadeIn(ans_bx), FadeIn(ans_title), run_time=0.2)
        ...
        self.play(Create(nl), FadeIn(VGroup(n2_bx, n2)), FadeIn(VGroup(n3_bx, n3)), run_time=0.4)
        self.play(FadeIn(e_dot, scale=0.5), FadeIn(VGroup(ne_bx, ne)), run_time=0.4)
        self.play(Flash(e_dot, color=GREEN, line_length=0.3, num_lines=10, run_time=0.3))

        # eˣ graph with matching gauges
        ...
        self.play(FadeIn(f_grid), Create(ax_f, run_time=0.3), Create(curve_f, run_time=0.4))

        # Quick slide showing height=slope
        ...
        self.play(x_f.animate.set_value(2.0), run_time=3.5, rate_func=smooth)
        ...
        self.play(FadeIn(punch_grid), run_time=0.15)
        ...
        self.play(FadeIn(r1_bx), Write(r1), run_time=0.4)
        self.wait(0.5)
        self.play(FadeIn(r2_bx), Write(r2), run_time=0.4)
        self.wait(0.5)
        self.play(FadeIn(rc2_bx), Write(recap_2), run_time=0.3)
        self.play(FadeIn(rc3_bx), Write(recap_3), run_time=0.3)
        self.play(FadeIn(rce_bx), Write(recap_e), run_time=0.3)
        self.play(Circumscribe(rce_bx, color=GREEN, run_time=0.5))
        ...
        self.wait(1.5)
        self.play(FadeOut(VGroup(logo, wordmark, tagline, end_grid)), run_time=0.3)
