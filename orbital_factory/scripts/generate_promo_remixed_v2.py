#!/usr/bin/env python3
"""Generate promo TTS — remixed voice, no prefix, no post-processing."""
...
OUTPUT_DIR = Path(os.path.expanduser("~/Desktop/Orbital/orbital_factory/output/tts/promo_remixed_v2"))
...
def generate(take_num):
    ...
    out = OUTPUT_DIR / f"take{take_num}.mp3"
    out.write_bytes(resp.content)
    print(f"  ✅ {out} ({len(resp.content)/1024:.0f}KB)")

def main():
    print("PROMO v2 — No prefix, same settings")
