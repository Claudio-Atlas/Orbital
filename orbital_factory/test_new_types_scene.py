...
class SyncedShortScene(Scene):
    STEPS_DATA = [{'type': 'text', 'content': '\\text{3 objects — how many orders?}', 'duration': 3.5, 'audio_path': ''}, {'type': 'permutation_display', 'content': '', 'duration': 16.0, 'audio_path': '', 'permutation_display': {'objects': [{'label': 'A', 'color': '#22D3EE'}, {'label': 'B', 'color': '#39FF14'}, {'label': 'C', 'color': '#9333EA'}], 'permutations': 'all', 'show_count': True, 'shelf_visible': True, 'speed': 'normal'}}, {'type': 'text', 'content': '\\text{What about arranging nothing?}', 'duration': 3.5, 'audio_path': ''}, {'type': 'empty_shelf', 'content': '', 'duration': 8.0, 'audio_path': '', 'empty_shelf': {'shelf_style': 'bar', 'glow': True, 'glow_color': '#22D3EE', 'caption': 'The empty arrangement', 'pause_duration': 1.0}}]
    ...
    def construct(self):
        ...
        for i, step in enumerate(steps):
            ...
            if stype == "permutation_display":
                ...
                # Cycle through remaining permutations
                speed_map = {"slow": 0.8, "normal": 0.45, "fast": 0.2}
                swap_time = speed_map.get(speed, 0.45)
                available = duration - 3.0
                actual_swap_time = min(swap_time, available / max(len(all_perms) - 1, 1))
                ...
                for pi in range(1, len(all_perms)):
                    ...
                    self.wait(actual_swap_time * 0.3)
            ...
            if stype == "empty_shelf":
                ...
                remaining = max(0.3, duration - pause_dur - 3.3)
                self.wait(remaining)
