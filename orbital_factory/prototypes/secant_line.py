"""
Prototype: "Secant Lines" — Visual Experiments v2
==================================================
Better bridge: specific → relabel → rewrite → simplify → general
Render: cd orbital_factory && source venv/bin/activate && PATH="/Library/TeX/texbin:$PATH" manim render prototypes/secant_line.py SecantLine -o secant_proto.mp4 --format mp4 -r 1080,1920 --frame_rate 60
"""
from manim import *
import numpy as np
...
class SecantLine(Scene):
    def construct(self):
        ...
        # ═══ SCENE 1: Callback — Same curve from Video A ═══
        axes = Axes(
        ...
        pick_text = Text("Pick two points", font_size=26, color=WHITE)
        pick_text.move_to([0, 2.5, 0])

        self.play(Write(pick_text), run_time=0.5)
        self.play(FadeIn(dot1, scale=1.5), Write(pt1_label), run_time=0.6)
        self.play(Flash(dot1.get_center(), color=GREEN, num_lines=6), run_time=0.3)
        self.play(FadeIn(dot2, scale=1.5), Write(pt2_label), run_time=0.6)
        self.play(Flash(dot2.get_center(), color=ORANGE, num_lines=6), run_time=0.3)
        self.wait(1.5)
        self.play(FadeOut(pick_text), run_time=0.3)

        # ═══ SCENE 3: Draw the secant line ═══
        ...
        rise_run_title = Text("Rise over Run", font_size=26, color=WHITE)
        rise_run_title.move_to([0, 2.5, 0])

        self.play(Write(rise_run_title), run_time=0.5)
        ...
        self.wait(2.0)
        self.play(FadeOut(rise_run_title), run_time=0.3)

        # ═══ SCENE 5: Specific formula ═══
        formula = MathTex(
        ...
        # Transform old labels to new ones
        self.play(Transform(pt1_label, x_label), run_time=0.8)
        self.wait(0.5)
        self.play(Transform(pt2_label, xh_label), run_time=0.8)
        self.wait(1.0)

        # ═══ SCENE 6b: Show h = the gap ═══
        # Draw a bracket between x=1 and x=3 on the x-axis
        h_line = Line(
        ...
        # Clean up h annotation and labels
        self.play(
        ...
        # ═══ SCENE 8: The tease — orange dot slides toward green ═══
        tease_text = Text("But this is the AVERAGE slope...", font_size=22, color=WHITE)
        tease_text.move_to([0, 2.5, 0])
        ...
        # Remove static elements, add dynamic ones
        self.remove(dot2, pt2_label, sec_line)
        self.add(moving_dot, moving_secant)

        self.play(h_tracker.animate.set_value(0.3), run_time=3.5, rate_func=smooth)

        tease_text2 = Text("What if h → 0?", font_size=28, color=GREEN, weight=BOLD)
        tease_text2.move_to([0, 3.2, 0])
        self.play(FadeOut(tease_text), Write(tease_text2), run_time=0.6)
        self.wait(2.0)

        # ═══ CLEANUP & END CARD ═══
        self.play(
            FadeOut(tease_text2), FadeOut(graph_group),
