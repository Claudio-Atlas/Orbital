      ...
      <main className="max-w-3xl mx-auto px-6 py-8 space-y-8">
        <section className={`rounded-2xl p-6 ${isDark ? "bg-white/5 border border-white/10" : "bg-white border border-gray-200"}`}>
          ...
          <div className="space-y-4">
            <div>
              <label className={`text-xs font-medium uppercase tracking-widest ${isDark ? "text-gray-500" : "text-gray-400"}`}>Display Name</label>
              <div className="mt-2 flex gap-2">
                <input type="text" value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="Your name" className={`flex-1 px-4 py-2.5 rounded-xl transition-all ${isDark ? "bg-black/40 text-white placeholder-gray-600 border border-white/10 focus:border-violet-500/50" : "bg-gray-50 text-gray-900 placeholder-gray-400 border border-gray-200 focus:border-violet-400"} focus:outline-none`} />
                <button onClick={handleSaveName} disabled={isSavingName} className="px-4 py-2.5 rounded-xl font-medium bg-violet-600 hover:bg-violet-500 text-white disabled:opacity-50 transition-all">
                  {isSavingName ? "..." : nameSuccess ? "✓" : "Save"}
                </button>
              </div>
              <p className={`mt-1.5 text-xs ${isDark ? "text-gray-600" : "text-gray-400"}`}>Used to personalize your video explanations</p>
            </div>
            <div>
              <label className={`text-xs font-medium uppercase tracking-widest ${isDark ? "text-gray-500" : "text-gray-400"}`}>Email</label>
              <p className="mt-1 font-medium">{user?.email}</p>
            </div>
            <div>
              <label className={`text-xs font-medium uppercase tracking-widest ${isDark ? "text-gray-500" : "text-gray-400"}`}>Minutes Balance</label>
              <p className="mt-1">
            ...
            <div>
              <label className={`text-xs font-medium uppercase tracking-widest ${isDark ? "text-gray-500" : "text-gray-400"}`}>Member Since</label>
        ...
        <section className={`rounded-2xl p-6 ${isDark ? "bg-white/5 border border-white/10" : "bg-white border border-gray-200"}`}>
          ...
          <p className={`mb-4 text-sm ${isDark ? "text-gray-400" : "text-gray-500"}`}>Choose the default voice for your video explanations.</p>
          <div className="grid grid-cols-2 gap-3">
            {VOICES.map((voice) => (
              <button key={voice.id} onClick={() => handleSaveVoice(voice.id)} className={`flex items-center gap-3 p-4 rounded-xl text-left transition-all ${defaultVoice === voice.id ? (isDark ? "bg-violet-600/20 border-2 border-violet-500" : "bg-violet-50 border-2 border-violet-500") : (isDark ? "bg-white/5 border border-white/10 hover:border-white/20" : "bg-gray-50 border border-gray-200 hover:border-gray-300")}`}>
                <span className="text-2xl">{voice.avatar}</span>
                <div className="flex-1">
                  <p className="font-medium">{voice.name}</p>
                  <p className={`text-xs ${isDark ? "text-gray-500" : "text-gray-400"}`}>{voice.desc}</p>
                </div>
                {defaultVoice === voice.id && <span className="text-violet-500">{Icons.check}</span>}
              </button>
            ))}
          </div>
        </section>
        ...
        <section className={`rounded-2xl p-6 ${isDark ? "bg-white/5 border border-white/10" : "bg-white border border-gray-200"}`}>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-sm ${isDark ? "text-gray-400" : "text-gray-500"}`}>{isDark ? "Dark mode" : "Light mode"}</p>
            </div>
            <button onClick={toggleTheme} className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${isDark ? "bg-white/10 hover:bg-white/15" : "bg-gray-100 hover:bg-gray-200"}`}>
        ...
        <section className={`rounded-2xl p-6 border ${isDark ? "border-red-500/20" : "border-red-200"}`}>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-sm ${isDark ? "text-gray-400" : "text-gray-500"}`}>Permanently delete your account and all data</p>
            </div>
            <button onClick={() => setShowDeleteModal(true)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${isDark ? "bg-red-500/10 text-red-400 hover:bg-red-500/20" : "bg-red-50 text-red-600 hover:bg-red-100"}`}>
              Delete Account
            </button>
          </div>
        </section>
