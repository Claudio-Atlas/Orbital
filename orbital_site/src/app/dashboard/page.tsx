...
// Video retention (same for all paying customers) — used for display in future
// const VIDEO_RETENTION_DAYS = 3; // All videos auto-delete after 3 days

// Voice options — each has custom post-processing for natural sound
const VOICES = [
  { 
    id: "clayton", 
    name: "Clayton", 
  ...
  { 
    id: "sarah", 
    name: "Sarah", 
    desc: "Bright & engaging",
    avatar: "👩‍🔬",
  },
];

// Video type for state
type VideoItem = {
  id: string;
  problem: string;
...
export default function DashboardPage() {
  ...
  const [_loadingVideos, setLoadingVideos] = useState(true); // For future loading state UI
  const [showPricingModal, setShowPricingModal] = useState(false);

  const _router = useRouter(); // Reserved for future navigation needs
  const { user, profile, loading: authLoading, signOut } = useAuth();
  const supabase = getSupabase();

  // Handle bfcache restore (back from Stripe)
  useEffect(() => {
    const handlePageShow = (e: PageTransitionEvent) => {
      if (e.persisted) {
        // Force hard navigation to reset all state (bfcache restores stale JS)
  ...
  // Note: Redirect for unauthenticated users is now handled by middleware
    ...
    async function fetchVideos() {
      ...
      try {
        ...
        if (data && data.length > 0) {
          // Map Supabase response to our VideoItem type
        ...
        // No videos yet - that's okay, empty state will show
      } catch (err) {
  ...
  const handleSignOut = async () => {
    await signOut();
    // Navigate to home - middleware will handle redirect if needed
  ...
  // Don't redirect here - it interferes with the signOut flow
  if (!user) {
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-black">
        <div className="w-8 h-8 border-2 border-violet-500 border-t-transparent rounded-full animate-spin" />
          ...
          <div
            className={`relative rounded-2xl ${
              isDark
                ...
                : "bg-white border border-gray-200 focus-within:border-violet-500/50"
            } transition-all duration-300 focus-within:shadow-[0_0_30px_rgba(139,92,246,0.15)]`}
          >
            <textarea
              value={problem}
              ...
              rows={3}
              className={`w-full px-5 py-4 bg-transparent resize-none focus:outline-none text-lg ${
                isDark
        ...
        <div>
          ...
          <div className="flex gap-4 overflow-x-auto pb-4 -mx-6 px-6 scrollbar-hide">
            {videos.map((video) => (
              <div
                key={video.id}
                className={`flex-shrink-0 w-64 rounded-xl overflow-hidden transition-all hover:scale-[1.02] cursor-pointer group ${
                  isDark
              ...
              >
                ...
                >
                  ...
                  {/* Play Button Overlay */}
                  <div
                    className={`relative z-10 w-14 h-14 rounded-full flex items-center justify-center transition-all group-hover:scale-110 ${
                      isDark
                  ...
                  <div
                    className={`absolute bottom-2 right-2 z-10 px-2 py-0.5 rounded text-xs font-medium ${
                      isDark ? "bg-black/60 text-white" : "bg-black/70 text-white"
                    }`}
                  >
            ...
            {/* View All Card */}
            <div
              className={`flex-shrink-0 w-64 h-[13.5rem] rounded-xl flex flex-col items-center justify-center transition-all cursor-pointer ${
                isDark
        ...
        <div className={`mt-8 grid grid-cols-3 gap-4`}>
          ...
          >
            <p className="text-3xl font-bold text-violet-500">
              {videos.reduce((acc, v) => acc + v.minutesUsed, 0).toFixed(1)}
            </p>
