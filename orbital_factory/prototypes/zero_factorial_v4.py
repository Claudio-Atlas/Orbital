...
Render:
  ...
  PATH="/Library/TeX/texbin:$PATH" manim render prototypes/zero_factorial_v4.py ZeroFactorialV4 \
    -o zero_factorial_v4.mp4 --format mp4 -r 1080,1920 --frame_rate 60 --flush_cache
"""
from manim import *
...
class ZeroFactorialV4(Scene):
    def construct(self):
        ...
        # Hook — needs to be BIG and punchy (short expression, needs larger font)
        hook_inner = MathTex(r"0! = 1 \text{ ?}", font_size=44, color=WHITE)
        _clamp(hook_inner, MAX_WIDTH * 0.85)
        hook_box = SurroundingRectangle(
            hook_inner, color=BOX_BORDER, fill_color=BOX_FILL,
            fill_opacity=0.6, buff=0.3, corner_radius=0.1, stroke_width=2,
        )
        mob1 = VGroup(hook_box, hook_inner)
        ...
        result_3 = MathTex(r"3! = 6", font_size=FS_ANSWER, color=GREEN)
        result_3.next_to(shelf_3, DOWN, buff=0.25)
        _clamp(result_3)
        ...
        result_2 = MathTex(r"2! = 2", font_size=FS_ANSWER, color=GREEN)
        result_2.next_to(shelf_2, DOWN, buff=0.25)
        _clamp(result_2)
        ...
        result_1 = MathTex(r"1! = 1", font_size=FS_ANSWER, color=GREEN)
        result_1.next_to(shelf_1, DOWN, buff=0.25)
        _clamp(result_1)
        ...
        empty_shelf_w = 2.8  # wider shelf — fills more space
        empty_shelf_bar = Line(
            LEFT * empty_shelf_w/2, RIGHT * empty_shelf_w/2,
            color=GREY_B, stroke_width=2.5
        )
        empty_shelf_bar.move_to([0, empty_shelf_y - 0.45, 0])
        ...
        # ═══════════════════════════════════════════════════════════
        glow_rect = Rectangle(
            width=empty_shelf_w * 0.85, height=0.8,
            color=CYAN, fill_color=CYAN,
            fill_opacity=0.0, stroke_width=0, stroke_opacity=0
        )
        glow_rect.move_to([0, empty_shelf_y + 0.1, 0])
        glow_ring = RoundedRectangle(
            width=empty_shelf_w * 0.9, height=0.9,
            color=CYAN, stroke_width=2, stroke_opacity=0.0,
            fill_opacity=0, corner_radius=0.12
        )
        glow_ring.move_to([0, empty_shelf_y + 0.1, 0])
        ...
        zero_q = MathTex(r"0! = \text{ ?}", font_size=40, color=WHITE)
        zero_q.move_to([0, empty_shelf_y + 1.0, 0])
        ...
        punch_inner = MathTex(r"0! = 1", font_size=42, color=GREEN)
